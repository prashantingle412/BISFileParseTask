...
func WriteData() {
	...
	for i, row := range reads {
	  if i == 1 {
		getId := []string{row[4]}
		substr := []rune(getId[0])
		if UserExist,isUserExistErr := isUserExist(string(substr[0:5])); isUserExistErr != nil{
			fmt.Println("User Info Already available with this Empid",isUserExistErr)
		}else if UserExist {		
	...
	if runnnerErr != nil {
		...
		fmt.Println("employee with this id is already exist in db")
	}		
   }
  }
 }		
...
func ShowDataById() {
	...
	if err != nil {
		...
		fmt.Println("Employee Details Are : ",m)
		fmt.Println("enter Attendees ID to fetch record:")
	 } 
	}
	if scanner.Err() != nil{
...
func isUserExist(id string) (bool,error)  {
	...
	if err != nil {
	  ...
	  return true,nil
		...
		fmt.Println("retruening true")
	  return false,nil
	}
  }
  
func main(){
	//  we used to write log in logger file of every transaction but for this iam skipping 
	WriteData()	
	ShowDataById()
}